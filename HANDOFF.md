...
## デザイン・構成の経緯（前身プロジェクトから流用）

本サイトは `C:\DOC\my-docs-site-zensical`（学習用に作ったZensical移行サイト）の
構成・デザインをそのまま土台として流用している。
...
- ロゴ/favicon: `docs/images/logo.png` / `favicon.png`（半導体・進捗管理イメージの
  二重正方形アイコン。Figmaファイル: https://www.figma.com/design/50MKuNkTabByO9prSzuHbg ）
- 各ページは「本文／改定履歴」タブ構成（`=== "本文"` / `=== "改定履歴"`）が記載ルール
- awesome-pagesプラグインの `.pages` ファイルでフォルダごとにタイトル・nav順を指定

## 章立て構成（全10章）

ユーザー指定の `基本設計書構造.md`（`C:\Users\t12cs\Downloads\` 配下、ローカルのみ）を
元に `docs/` 配下にスタブページを作成済み。各ページの内容はまだ未着手（`status: "未着手"`）。
...
## 関連メモ（このPCの `C:\Users\t12cs\.claude\projects\C--DOC\memory\` 内）

別PCのClaudeにはこのメモは見えないため、必要であれば本ファイルの内容を都度メモに
転記すること。

- `project_mkdocs_site.md` — my-docs-site / my-docs-site-zensical の構築経緯、
  Zensicalのプラグイン互換性調査、デザインの意思決定の詳細
- `feedback_dev.md` — 開発方針の好み（.md形式、フォルダ完結、技術文書は可読性優先 等）
